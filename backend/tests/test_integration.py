...
def test_chat_flow(client):
    ...
    client.post(
        f"/conversations/{conv_id}/messages",
        json={"text": "Gut und dir?"},
        headers=auth_header(token_b),
    )

    # Full message history
    msgs = client.get(
        f"/conversations/{conv_id}/messages", headers=auth_header(token_a)
    ).json()
    assert len(msgs) == 3
...
def test_save_flow(client):
    ...
    # Create collection
    col = client.post(
        "/collections", json={"name": "Lieblinge"}, headers=auth_header(token)
    ).json()

    # Add to collection
    client.post(
        f"/recipes/{recipe_id}/toggle-collection/{col['id']}",
        headers=auth_header(token),
    )
    ...
    col_vids = client.get(
        f"/collections/{col['id']}/videos", headers=auth_header(token)
    ).json()

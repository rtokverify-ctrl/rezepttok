...
def test_share_shopping_list(client, auth_token, second_auth_token):
    ...
    r = client.post(
        f"/shopping-lists/{list_id}/share",
        json={"username": "testuser2"},
        headers=auth_header(auth_token),
    )
    assert r.status_code == 200
    assert "Shared Party" in [
        lst["name"]
        for lst in client.get(
            "/shopping-lists", headers=auth_header(second_auth_token)
        ).json()
    ]
...
def test_shared_list_access(client, auth_token, second_auth_token):
    ...
    list_resp = client.post(
        "/shopping-lists", json={"name": "WG Einkauf"}, headers=auth_header(auth_token)
    ).json()
    list_id = list_resp["id"]
    client.post(
    ...
    user_b_lists = client.get(
        "/shopping-lists", headers=auth_header(second_auth_token)
    ).json()
    has_shared_list = any(
        lst["id"] == list_id and lst["is_shared"] is True for lst in user_b_lists
    )
    assert has_shared_list is True

    # 4. User B should be able to see the items and toggle them
    items = client.get(
        f"/shopping-lists/{list_id}/items", headers=auth_header(second_auth_token)
    ).json()
    assert len(items) == 1
    ...
    item_id = items[0]["id"]
    t = client.post(
        f"/shopping-lists/items/{item_id}/toggle",

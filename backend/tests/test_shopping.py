...
def test_add_item_to_list(client, auth_token):
    ...
    list_resp = client.post(
        "/shopping-lists", json={"name": "Party"}, headers=auth_header(auth_token)
    ).json()
    ...
    r = client.post(
        ...
        json={"item": "Bier", "quantity": "2 Kasten"},
        headers=auth_header(auth_token),
    )
    data = r.json()
    assert r.status_code == 200
...
def test_update_item(client, auth_token):
    ...
    list_resp = client.post(
        "/shopping-lists", json={"name": "Update Test"}, headers=auth_header(auth_token)
    ).json()
    item_resp = client.post(
        f"/shopping-lists/{list_resp['id']}/items",
        json={"item": "Mehl", "quantity": "1kg"},
        headers=auth_header(auth_token),
    ).json()

    # 2. Update quantity
    r = client.patch(
        f"/shopping-lists/items/{item_resp['id']}",
        json={"quantity": "2kg"},
        headers=auth_header(auth_token),
    )
    assert r.status_code == 200
...
def test_toggle_item(client, auth_token):
    lst = client.post(
        "/shopping-lists", json={"name": "Test"}, headers=auth_header(auth_token)
    ).json()
    item = client.post(
    ...
    r = client.post(
        f"/shopping-lists/items/{item['id']}/toggle", headers=auth_header(auth_token)
    )
...
def test_delete_item(client, auth_token):
    lst = client.post(
        "/shopping-lists", json={"name": "Test 2"}, headers=auth_header(auth_token)
    ).json()
    item = client.post(
        f"/shopping-lists/{lst['id']}/items",
        json={"item": "Eier"},
    ...
    r = client.delete(
        f"/shopping-lists/items/{item['id']}", headers=auth_header(auth_token)
    )
    assert r.status_code == 200

    items = client.get(
        f"/shopping-lists/{lst['id']}/items", headers=auth_header(auth_token)
    ).json()
    assert len(items) == 0
...
def test_multiple_items(client, auth_token):
    lst = client.post(
        "/shopping-lists", json={"name": "Backen"}, headers=auth_header(auth_token)
    ).json()
    client.post(
        f"/shopping-lists/{lst['id']}/items",
        json={"item": "Mehl"},

...
def get_feed_with_edge_rank(
    ...
    likes_sub = select(func.count(Like.user_id)).where(
        Like.recipe_id == Recipe.id).scalar_subquery()
    saves_sub = select(func.count(SavedRecipe.user_id)).where(
        SavedRecipe.recipe_id == Recipe.id).scalar_subquery()
    comments_sub = select(func.count(Comment.id)).where(
        Comment.recipe_id == Recipe.id).scalar_subquery()
    ...
    base_score = (c_likes * 2) + (c_saves * 3) + \
        (c_comments * 2) + (c_views * 0.1)

    # Age calculation
    if getattr(engine.dialect, 'name', '') == 'sqlite':
        # SQLite compatible age calculation (in hours)
        age_hours = (func.julianday('now') -
                     func.julianday(Recipe.created_at)) * 24
        ...
        # then subtract from now(), get epoch seconds
        age_seconds = func.extract(
            'epoch', func.now() - cast(Recipe.created_at, DateTime))
    ...
    is_following = select(Follow.follower_id).where(
        and_(Follow.follower_id == current_user_id,
             Follow.following_id == Recipe.owner_id)
    ).exists()

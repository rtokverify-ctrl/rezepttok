...
def get_feed(
    ...
    nextCursor = (
        f"{ranked_recipes[-1][1]}_{ranked_recipes[-1][0].id}"
        if ranked_recipes
        else None
    )
...
def get_all_saved_videos(
    db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    # Distinct recipe_ids holen
    saves = (
        db.query(SavedRecipe.recipe_id)
...
def toggle_collection_save(
    ...
    if existing:
        ...
        db.add(
            SavedRecipe(
                user_id=user.id, recipe_id=recipe_id, collection_id=collection_id
            )
        )
        active = True
...
async def toggle_like(
    ...
    existing = (
        db.query(Like)
        .filter(Like.user_id == user.id, Like.recipe_id == recipe_id)
        .first()
    )
    if existing:
        db.delete(existing)
        ...
        if recipe and recipe.owner_id != user.id:
            ...
            await send_push_notification(
                ...
                recipe.owner_id,
                "Neuer Like",
                f"{user.username} gefällt dein Rezept!",
            )
        else:
            db.commit()
    return {"msg": "Ok"}
...
def get_comments(recipe_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    def format_comment(c):
        ...
        return {
            ...
            else (u.username if u else "Gast"),
            "user_id": c.user_id,
...
async def create_comment(
    ...
    if comment.parent_id:
        ...
        if (
            parent_comment
            and parent_comment.user_id != user.id
            and (not recipe or parent_comment.user_id != recipe.owner_id)
        ):
            db.add(
                Notification(
    ...
    if recipe and recipe.owner_id != user.id:
        await send_push_notification(
            ...
            recipe.owner_id,
            "Neuer Kommentar",
            f"{user.username} hat kommentiert: {comment.text}",
        )
    if comment.parent_id:
        parent_comment = (
            db.query(Comment).filter(Comment.id == comment.parent_id).first()
...
def update_recipe(
    ...
    if recipe.owner_id != user.id:
        raise HTTPException(
            status_code=403, detail="Nur der Ersteller kann dieses Rezept bearbeiten"
        )
...
def delete_recipe(
    ...
    if not recipe or recipe.owner_id != user.id:
        raise HTTPException(403, "Fehler")
    if os.path.exists(recipe.video_url.lstrip("/")):
        os.remove(recipe.video_url.lstrip("/"))
    db.delete(recipe)
    db.commit()
    return {"msg": "Weg"}
...
def get_trending_recipes(
    ...
    recipes = (
        db.query(Recipe)
        .order_by(Recipe.views.desc().nullslast(), Recipe.created_at.desc())
        .limit(15)
        .all()
    )
...
def get_recipes_by_tag(
    ...
    recipes = (
        db.query(Recipe)
        .filter(cast(Recipe.tags, String).ilike(f"%{tag}%"))

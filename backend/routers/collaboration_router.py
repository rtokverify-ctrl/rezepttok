...
def share_list(
    ...
    existing = (
        ...
        .filter(
            SharedShoppingList.list_id == list_id,
            SharedShoppingList.shared_with_user_id == target_user.id,
        )
        .first()
    )
    if existing:
...
def get_shared_shopping_lists(
    ...
    shared = (
        db.query(SharedShoppingList)
        .filter(SharedShoppingList.shared_with_user_id == current_user.id)
        .all()
    )
    results = []
    for s in shared:
        lst = db.query(ShoppingList).filter(ShoppingList.id == s.list_id).first()
        if lst:
...
def share_collection(
    ...
    if not c:
        raise HTTPException(
            status_code=404, detail="Collection not found or unauthorized"
        )
    ...
    existing = (
        ...
        .filter(
            ...
            SharedCollection.shared_with_user_id == target_user.id,
        )
        .first()
    )
    if existing:
    ...
    share = SharedCollection(
        collection_id=collection_id, shared_with_user_id=target_user.id
    )
    db.add(share)
    db.commit()
...
def get_shared_collections(
    ...
    shared = (
        db.query(SharedCollection)
        .filter(SharedCollection.shared_with_user_id == current_user.id)
        .all()
    )
    results = []
    for s in shared:

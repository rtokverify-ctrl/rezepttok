        ...
        results.append(
            {
                ...
                "chef": current_user.display_name,
                "owner_id": r.owner_id,
...
async def toggle_follow(
    ...
    if existing:
        ...
        await send_push_notification(
            db, user_id, "Neuer Follow", f"{current_user.username} folgt dir jetzt!"
        )
...
def get_user_profile(
    ...
    recipes = (
        db.query(Recipe)
        .filter(Recipe.owner_id == user_id)
        .order_by(Recipe.id.desc())
        .all()
    )
    videos = []
...
def get_my_saved_videos(
    ...
    for s in saved:
        if s.recipe_id in seen_recipe_ids:
            continue
...
async def delete_my_profile(
    ...
    db.query(SavedRecipe).filter(SavedRecipe.user_id == current_user.id).delete(
        synchronize_session=False
    )

    # 5. Chats / Conversations / Messages
    db.query(Message).filter(Message.sender_id == current_user.id).delete(
        synchronize_session=False
    )
    user_convos = (
    ...
    # 6. Collections
    db.query(SharedCollection).filter(
        SharedCollection.shared_with_user_id == current_user.id
    ).delete(synchronize_session=False)
    user_collections = (
        db.query(Collection).filter(Collection.user_id == current_user.id).all()
    )
    for col in user_collections:
        db.query(SavedRecipe).filter(SavedRecipe.collection_id == col.id).delete(
            synchronize_session=False
        )
        db.query(SharedCollection).filter(
            SharedCollection.collection_id == col.id
        ).delete(synchronize_session=False)
        db.delete(col)

    # 7. ShoppingLists
    db.query(SharedShoppingList).filter(
        SharedShoppingList.shared_with_user_id == current_user.id
    ).delete(synchronize_session=False)
    user_lists = (
        db.query(ShoppingList).filter(ShoppingList.user_id == current_user.id).all()
    )
    for slist in user_lists:
        db.query(ShoppingListItem).filter(ShoppingListItem.list_id == slist.id).delete(
            synchronize_session=False
        )
        db.query(SharedShoppingList).filter(
            SharedShoppingList.list_id == slist.id
        ).delete(synchronize_session=False)
        db.delete(slist)
    ...
    for r in my_recipes:
        ...
        db.query(Comment).filter(Comment.recipe_id == r.id).delete(
            synchronize_session=False
        )
        db.query(SavedRecipe).filter(SavedRecipe.recipe_id == r.id).delete(
            synchronize_session=False
        )
        db.delete(r)
